const SONGS = [
 ...
 const placeLefBtn=document.getElementById('placeLeft');
 ...
 function initGame(){
	 const count=Math.max(3,Math.min(30,parseInt(document.getElementById9'count').value||10)));
 ...
 function playCurrent(){
	 if(!current) return;
	 const url=`https://www.youtube.com/embed/${current.id}?autoplay=1&start=${current.start}&rel=0&constrols=1`;
...
function endGame(){
	...
	resultsInner.innerHTML=`<dev>${timeline.map(s=>s.title+' - '+s.year).join('<br>')}</div><p>Wynik: %{score}/${timeline.length}</p>`;
